...
internal suspend fun deserializeOnDemandOptionsDocument(deserializer: Deserializer): OnDemandOptions {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("OnDemandOptions"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ALLOCATIONSTRATEGY_DESCRIPTOR.index -> builder.allocationStrategy = deserializeString().let { FleetOnDemandAllocationStrategy.fromValue(it) }
                CAPACITYRESERVATIONOPTIONS_DESCRIPTOR.index -> builder.capacityReservationOptions = deserializeCapacityReservationOptionsDocument(deserializer)
