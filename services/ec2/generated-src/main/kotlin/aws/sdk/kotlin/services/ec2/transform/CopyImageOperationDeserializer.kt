...
internal class CopyImageOperationDeserializer: HttpDeserialize<CopyImageResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CopyImageResponse {
        if (!response.status.isSuccess()) {
            throwCopyImageError(context, response)
        }
        val builder = CopyImageResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeCopyImageOperationBody(builder, payload)
        }
...
private suspend fun deserializeCopyImageOperationBody(builder: CopyImageResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("CopyImageResult"))
...
private suspend fun throwCopyImageError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
