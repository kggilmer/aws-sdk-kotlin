...
import aws.sdk.kotlin.services.ec2.model.ElasticGpuStatus
...
internal suspend fun deserializeElasticGpuHealthDocument(deserializer: Deserializer): ElasticGpuHealth {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("ElasticGpuHealth"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                STATUS_DESCRIPTOR.index -> builder.status = deserializeString().let { ElasticGpuStatus.fromValue(it) }
                null -> break@loop
                else -> skipValue()
            }
        }
