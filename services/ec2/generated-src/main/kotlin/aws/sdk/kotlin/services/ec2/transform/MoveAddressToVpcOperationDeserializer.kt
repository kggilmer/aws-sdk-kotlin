...
internal class MoveAddressToVpcOperationDeserializer: HttpDeserialize<MoveAddressToVpcResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): MoveAddressToVpcResponse {
        if (!response.status.isSuccess()) {
            throwMoveAddressToVpcError(context, response)
        }
        val builder = MoveAddressToVpcResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeMoveAddressToVpcOperationBody(builder, payload)
        }
...
private suspend fun deserializeMoveAddressToVpcOperationBody(builder: MoveAddressToVpcResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("MoveAddressToVpcResult"))
...
private suspend fun throwMoveAddressToVpcError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
