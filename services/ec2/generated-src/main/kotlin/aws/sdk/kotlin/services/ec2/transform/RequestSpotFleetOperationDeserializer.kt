...
internal class RequestSpotFleetOperationDeserializer: HttpDeserialize<RequestSpotFleetResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): RequestSpotFleetResponse {
        if (!response.status.isSuccess()) {
            throwRequestSpotFleetError(context, response)
        }
        val builder = RequestSpotFleetResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeRequestSpotFleetOperationBody(builder, payload)
        }
...
private suspend fun deserializeRequestSpotFleetOperationBody(builder: RequestSpotFleetResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("RequestSpotFleetResult"))
...
private suspend fun throwRequestSpotFleetError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
