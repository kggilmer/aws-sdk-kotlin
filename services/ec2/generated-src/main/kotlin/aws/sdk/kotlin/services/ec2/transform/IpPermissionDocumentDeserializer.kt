...
internal suspend fun deserializeIpPermissionDocument(deserializer: Deserializer): IpPermission {
    ...
    val IPV6RANGES_DESCRIPTOR = SdkFieldDescriptor(SerialKind.List, XmlSerialName("ipv6Ranges"), XmlCollectionName("item"))
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("IpPermission"))
            ...
            when (findNextFieldIndex()) {
                ...
                IPRANGES_DESCRIPTOR.index -> builder.ipRanges =
                    deserializer.deserializeList(IPRANGES_DESCRIPTOR) {
                        val col0 = mutableListOf<IpRange>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializeIpRangeDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
                IPV6RANGES_DESCRIPTOR.index -> builder.ipv6Ranges =
                    deserializer.deserializeList(IPV6RANGES_DESCRIPTOR) {
                        val col0 = mutableListOf<Ipv6Range>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializeIpv6RangeDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
                PREFIXLISTIDS_DESCRIPTOR.index -> builder.prefixListIds =
                    deserializer.deserializeList(PREFIXLISTIDS_DESCRIPTOR) {
                        val col0 = mutableListOf<PrefixListId>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializePrefixListIdDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
