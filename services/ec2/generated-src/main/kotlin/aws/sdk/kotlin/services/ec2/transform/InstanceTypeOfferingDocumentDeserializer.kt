...
internal suspend fun deserializeInstanceTypeOfferingDocument(deserializer: Deserializer): InstanceTypeOffering {
    ...
    val LOCATIONTYPE_DESCRIPTOR = SdkFieldDescriptor(SerialKind.String, XmlSerialName("locationType"))
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("InstanceTypeOffering"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ...
                LOCATIONTYPE_DESCRIPTOR.index -> builder.locationType = deserializeString().let { LocationType.fromValue(it) }
                null -> break@loop
                else -> skipValue()
            }
        }
