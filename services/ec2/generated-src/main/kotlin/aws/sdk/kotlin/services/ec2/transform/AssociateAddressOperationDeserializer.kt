...
internal class AssociateAddressOperationDeserializer: HttpDeserialize<AssociateAddressResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): AssociateAddressResponse {
        if (!response.status.isSuccess()) {
            throwAssociateAddressError(context, response)
        }
        val builder = AssociateAddressResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeAssociateAddressOperationBody(builder, payload)
        }
...
private suspend fun deserializeAssociateAddressOperationBody(builder: AssociateAddressResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("AssociateAddressResult"))
...
private suspend fun throwAssociateAddressError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
