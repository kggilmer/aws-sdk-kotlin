...
internal class DeleteLaunchTemplateOperationDeserializer: HttpDeserialize<DeleteLaunchTemplateResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteLaunchTemplateResponse {
        if (!response.status.isSuccess()) {
            throwDeleteLaunchTemplateError(context, response)
        }
        val builder = DeleteLaunchTemplateResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteLaunchTemplateOperationBody(builder, payload)
        }
...
private suspend fun deserializeDeleteLaunchTemplateOperationBody(builder: DeleteLaunchTemplateResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteLaunchTemplateResult"))
...
private suspend fun throwDeleteLaunchTemplateError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
