...
internal class PurchaseHostReservationOperationDeserializer: HttpDeserialize<PurchaseHostReservationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PurchaseHostReservationResponse {
        if (!response.status.isSuccess()) {
            throwPurchaseHostReservationError(context, response)
        }
        val builder = PurchaseHostReservationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializePurchaseHostReservationOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializePurchaseHostReservationOperationBody(builder: PurchaseHostReservationResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("PurchaseHostReservationResult"))
...
private suspend fun throwPurchaseHostReservationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
