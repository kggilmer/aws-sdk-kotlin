...
internal suspend fun deserializeLaunchTemplateInstanceMarketOptionsDocument(deserializer: Deserializer): LaunchTemplateInstanceMarketOptions {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("LaunchTemplateInstanceMarketOptions"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                MARKETTYPE_DESCRIPTOR.index -> builder.marketType = deserializeString().let { MarketType.fromValue(it) }
                SPOTOPTIONS_DESCRIPTOR.index -> builder.spotOptions = deserializeLaunchTemplateSpotMarketOptionsDocument(deserializer)
                null -> break@loop
                else -> skipValue()
            }
        }
