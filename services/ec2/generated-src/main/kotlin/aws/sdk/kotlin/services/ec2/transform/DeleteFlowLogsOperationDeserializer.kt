...
internal class DeleteFlowLogsOperationDeserializer: HttpDeserialize<DeleteFlowLogsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteFlowLogsResponse {
        if (!response.status.isSuccess()) {
            throwDeleteFlowLogsError(context, response)
        }
        val builder = DeleteFlowLogsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteFlowLogsOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializeDeleteFlowLogsOperationBody(builder: DeleteFlowLogsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteFlowLogsResult"))
...
private suspend fun throwDeleteFlowLogsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
