...
internal class RejectVpcEndpointConnectionsOperationDeserializer: HttpDeserialize<RejectVpcEndpointConnectionsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): RejectVpcEndpointConnectionsResponse {
        if (!response.status.isSuccess()) {
            throwRejectVpcEndpointConnectionsError(context, response)
        }
        val builder = RejectVpcEndpointConnectionsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeRejectVpcEndpointConnectionsOperationBody(builder, payload)
        }
...
private suspend fun deserializeRejectVpcEndpointConnectionsOperationBody(builder: RejectVpcEndpointConnectionsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("RejectVpcEndpointConnectionsResult"))
...
private suspend fun throwRejectVpcEndpointConnectionsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
