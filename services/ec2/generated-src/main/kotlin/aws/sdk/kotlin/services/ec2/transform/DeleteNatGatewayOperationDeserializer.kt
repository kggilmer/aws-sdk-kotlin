...
internal class DeleteNatGatewayOperationDeserializer: HttpDeserialize<DeleteNatGatewayResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteNatGatewayResponse {
        if (!response.status.isSuccess()) {
            throwDeleteNatGatewayError(context, response)
        }
        val builder = DeleteNatGatewayResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteNatGatewayOperationBody(builder, payload)
        }
...
private suspend fun deserializeDeleteNatGatewayOperationBody(builder: DeleteNatGatewayResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteNatGatewayResult"))
...
private suspend fun throwDeleteNatGatewayError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
