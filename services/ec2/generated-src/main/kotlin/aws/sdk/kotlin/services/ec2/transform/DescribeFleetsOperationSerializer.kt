...
internal class DescribeFleetsOperationSerializer: HttpSerialize<DescribeFleetsRequest> {
    override suspend fun serialize(context: ExecutionContext, input: DescribeFleetsRequest): HttpRequestBuilder {
        ...
        val payload = serializeDescribeFleetsOperationBody(context, input)
...
private fun serializeDescribeFleetsOperationBody(context: ExecutionContext, input: DescribeFleetsRequest): ByteArray {
    ...
    val FLEETIDS_DESCRIPTOR = SdkFieldDescriptor(SerialKind.List, FormUrlSerialName("FleetId"), FormUrlFlattened)
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(FormUrlSerialName("DescribeFleetsRequest"))
        trait(QueryLiteral("Action", "DescribeFleets"))
        trait(QueryLiteral("Version", "2016-11-15"))
        field(DRYRUN_DESCRIPTOR)
        field(FILTERS_DESCRIPTOR)
        field(FLEETIDS_DESCRIPTOR)
    ...
    serializer.serializeStruct(OBJ_DESCRIPTOR) {
        ...
        if (input.fleetIds != null) {
            listField(FLEETIDS_DESCRIPTOR) {
                for (el0 in input.fleetIds) {
                    serializeString(el0)
                }
            }
        }
