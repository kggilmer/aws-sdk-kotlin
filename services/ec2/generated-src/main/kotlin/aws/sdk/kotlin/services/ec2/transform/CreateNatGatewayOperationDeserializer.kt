...
internal class CreateNatGatewayOperationDeserializer: HttpDeserialize<CreateNatGatewayResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateNatGatewayResponse {
        if (!response.status.isSuccess()) {
            throwCreateNatGatewayError(context, response)
        }
        val builder = CreateNatGatewayResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeCreateNatGatewayOperationBody(builder, payload)
        }
...
private suspend fun deserializeCreateNatGatewayOperationBody(builder: CreateNatGatewayResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("CreateNatGatewayResult"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ...
                NATGATEWAY_DESCRIPTOR.index -> builder.natGateway = deserializeNatGatewayDocument(deserializer)
                null -> break@loop
                else -> skipValue()
            }
        }
...
private suspend fun throwCreateNatGatewayError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
