...
internal suspend fun deserializeAnalysisLoadBalancerListenerDocument(deserializer: Deserializer): AnalysisLoadBalancerListener {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("AnalysisLoadBalancerListener"))
        trait(XmlNamespace("http://ec2.amazonaws.com/doc/2016-11-15"))
        field(INSTANCEPORT_DESCRIPTOR)
        field(LOADBALANCERPORT_DESCRIPTOR)
    }

    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                INSTANCEPORT_DESCRIPTOR.index -> builder.instancePort = deserializeInt()
                LOADBALANCERPORT_DESCRIPTOR.index -> builder.loadBalancerPort = deserializeInt()
                null -> break@loop
                else -> skipValue()
            }
        }
