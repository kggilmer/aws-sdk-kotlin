...
internal class ReplaceRouteTableAssociationOperationDeserializer: HttpDeserialize<ReplaceRouteTableAssociationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): ReplaceRouteTableAssociationResponse {
        if (!response.status.isSuccess()) {
            throwReplaceRouteTableAssociationError(context, response)
        }
        val builder = ReplaceRouteTableAssociationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeReplaceRouteTableAssociationOperationBody(builder, payload)
        }
...
private suspend fun deserializeReplaceRouteTableAssociationOperationBody(builder: ReplaceRouteTableAssociationResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("ReplaceRouteTableAssociationResult"))
        field(ASSOCIATIONSTATE_DESCRIPTOR)
        field(NEWASSOCIATIONID_DESCRIPTOR)
    }
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ...
                NEWASSOCIATIONID_DESCRIPTOR.index -> builder.newAssociationId = deserializeString()
                null -> break@loop
                else -> skipValue()
            }
        }
...
private suspend fun throwReplaceRouteTableAssociationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
