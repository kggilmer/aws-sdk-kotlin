...
internal class UnmonitorInstancesOperationDeserializer: HttpDeserialize<UnmonitorInstancesResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): UnmonitorInstancesResponse {
        if (!response.status.isSuccess()) {
            throwUnmonitorInstancesError(context, response)
        }
        val builder = UnmonitorInstancesResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeUnmonitorInstancesOperationBody(builder, payload)
        }
...
private suspend fun deserializeUnmonitorInstancesOperationBody(builder: UnmonitorInstancesResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("UnmonitorInstancesResult"))
...
private suspend fun throwUnmonitorInstancesError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
