...
internal class ProvisionByoipCidrOperationDeserializer: HttpDeserialize<ProvisionByoipCidrResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): ProvisionByoipCidrResponse {
        if (!response.status.isSuccess()) {
            throwProvisionByoipCidrError(context, response)
        }
        val builder = ProvisionByoipCidrResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeProvisionByoipCidrOperationBody(builder, payload)
        }
...
private suspend fun deserializeProvisionByoipCidrOperationBody(builder: ProvisionByoipCidrResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("ProvisionByoipCidrResult"))
...
private suspend fun throwProvisionByoipCidrError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
