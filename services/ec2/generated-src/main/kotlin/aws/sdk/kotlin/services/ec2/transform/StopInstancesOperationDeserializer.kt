...
internal class StopInstancesOperationDeserializer: HttpDeserialize<StopInstancesResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): StopInstancesResponse {
        if (!response.status.isSuccess()) {
            throwStopInstancesError(context, response)
        }
        val builder = StopInstancesResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeStopInstancesOperationBody(builder, payload)
        }
...
private suspend fun deserializeStopInstancesOperationBody(builder: StopInstancesResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("StopInstancesResult"))
        field(STOPPINGINSTANCES_DESCRIPTOR)
    }

    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                STOPPINGINSTANCES_DESCRIPTOR.index -> builder.stoppingInstances =
                    deserializer.deserializeList(STOPPINGINSTANCES_DESCRIPTOR) {
...
private suspend fun throwStopInstancesError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
