...
internal class CreateFpgaImageOperationDeserializer: HttpDeserialize<CreateFpgaImageResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateFpgaImageResponse {
        if (!response.status.isSuccess()) {
            throwCreateFpgaImageError(context, response)
        }
        val builder = CreateFpgaImageResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeCreateFpgaImageOperationBody(builder, payload)
        }
...
private suspend fun deserializeCreateFpgaImageOperationBody(builder: CreateFpgaImageResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("CreateFpgaImageResult"))
...
private suspend fun throwCreateFpgaImageError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
