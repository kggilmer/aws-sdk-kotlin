...
internal class CreateVpcEndpointOperationDeserializer: HttpDeserialize<CreateVpcEndpointResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateVpcEndpointResponse {
        if (!response.status.isSuccess()) {
            throwCreateVpcEndpointError(context, response)
        }
        val builder = CreateVpcEndpointResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeCreateVpcEndpointOperationBody(builder, payload)
        }
...
private suspend fun deserializeCreateVpcEndpointOperationBody(builder: CreateVpcEndpointResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("CreateVpcEndpointResult"))
...
private suspend fun throwCreateVpcEndpointError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
