...
internal class ReleaseHostsOperationDeserializer: HttpDeserialize<ReleaseHostsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): ReleaseHostsResponse {
        if (!response.status.isSuccess()) {
            throwReleaseHostsError(context, response)
        }
        val builder = ReleaseHostsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeReleaseHostsOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializeReleaseHostsOperationBody(builder: ReleaseHostsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("ReleaseHostsResult"))
...
private suspend fun throwReleaseHostsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
