...
internal class DeleteVpcEndpointsOperationDeserializer: HttpDeserialize<DeleteVpcEndpointsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteVpcEndpointsResponse {
        if (!response.status.isSuccess()) {
            throwDeleteVpcEndpointsError(context, response)
        }
        val builder = DeleteVpcEndpointsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteVpcEndpointsOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializeDeleteVpcEndpointsOperationBody(builder: DeleteVpcEndpointsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteVpcEndpointsResult"))
...
private suspend fun throwDeleteVpcEndpointsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
