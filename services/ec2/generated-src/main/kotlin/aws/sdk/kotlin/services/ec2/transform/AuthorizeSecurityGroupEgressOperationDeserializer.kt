...
internal class AuthorizeSecurityGroupEgressOperationDeserializer: HttpDeserialize<AuthorizeSecurityGroupEgressResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): AuthorizeSecurityGroupEgressResponse {
        if (!response.status.isSuccess()) {
            throwAuthorizeSecurityGroupEgressError(context, response)
        }
        val builder = AuthorizeSecurityGroupEgressResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeAuthorizeSecurityGroupEgressOperationBody(builder, payload)
        }
...
private suspend fun deserializeAuthorizeSecurityGroupEgressOperationBody(builder: AuthorizeSecurityGroupEgressResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("AuthorizeSecurityGroupEgressResult"))
...
private suspend fun throwAuthorizeSecurityGroupEgressError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
