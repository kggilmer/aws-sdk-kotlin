...
import aws.sdk.kotlin.services.ec2.model.FleetReplacementStrategy
import aws.sdk.kotlin.services.ec2.model.FleetSpotCapacityRebalance
...
internal suspend fun deserializeFleetSpotCapacityRebalanceDocument(deserializer: Deserializer): FleetSpotCapacityRebalance {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("FleetSpotCapacityRebalance"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                REPLACEMENTSTRATEGY_DESCRIPTOR.index -> builder.replacementStrategy = deserializeString().let { FleetReplacementStrategy.fromValue(it) }
                null -> break@loop
                else -> skipValue()
            }
        }
