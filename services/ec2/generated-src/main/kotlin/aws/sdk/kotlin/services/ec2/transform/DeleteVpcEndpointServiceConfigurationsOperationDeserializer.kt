...
internal class DeleteVpcEndpointServiceConfigurationsOperationDeserializer: HttpDeserialize<DeleteVpcEndpointServiceConfigurationsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteVpcEndpointServiceConfigurationsResponse {
        if (!response.status.isSuccess()) {
            throwDeleteVpcEndpointServiceConfigurationsError(context, response)
        }
        val builder = DeleteVpcEndpointServiceConfigurationsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteVpcEndpointServiceConfigurationsOperationBody(builder, payload)
        }
...
private suspend fun deserializeDeleteVpcEndpointServiceConfigurationsOperationBody(builder: DeleteVpcEndpointServiceConfigurationsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteVpcEndpointServiceConfigurationsResult"))
...
private suspend fun throwDeleteVpcEndpointServiceConfigurationsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
