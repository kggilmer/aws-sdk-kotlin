...
internal class GetPasswordDataOperationDeserializer: HttpDeserialize<GetPasswordDataResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetPasswordDataResponse {
        if (!response.status.isSuccess()) {
            throwGetPasswordDataError(context, response)
        }
        val builder = GetPasswordDataResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetPasswordDataOperationBody(builder, payload)
        }
...
private suspend fun deserializeGetPasswordDataOperationBody(builder: GetPasswordDataResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("GetPasswordDataResult"))
        field(INSTANCEID_DESCRIPTOR)
        field(PASSWORDDATA_DESCRIPTOR)
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ...
                PASSWORDDATA_DESCRIPTOR.index -> builder.passwordData = deserializeString()
...
private suspend fun throwGetPasswordDataError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
