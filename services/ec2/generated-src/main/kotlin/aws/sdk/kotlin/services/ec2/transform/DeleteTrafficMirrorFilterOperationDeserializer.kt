...
internal class DeleteTrafficMirrorFilterOperationDeserializer: HttpDeserialize<DeleteTrafficMirrorFilterResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteTrafficMirrorFilterResponse {
        if (!response.status.isSuccess()) {
            throwDeleteTrafficMirrorFilterError(context, response)
        }
        val builder = DeleteTrafficMirrorFilterResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteTrafficMirrorFilterOperationBody(builder, payload)
        }
...
private suspend fun deserializeDeleteTrafficMirrorFilterOperationBody(builder: DeleteTrafficMirrorFilterResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DeleteTrafficMirrorFilterResult"))
...
private suspend fun throwDeleteTrafficMirrorFilterError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
