...
internal class AttachVpnGatewayOperationDeserializer: HttpDeserialize<AttachVpnGatewayResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): AttachVpnGatewayResponse {
        if (!response.status.isSuccess()) {
            throwAttachVpnGatewayError(context, response)
        }
        val builder = AttachVpnGatewayResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeAttachVpnGatewayOperationBody(builder, payload)
        }
...
private suspend fun deserializeAttachVpnGatewayOperationBody(builder: AttachVpnGatewayResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("AttachVpnGatewayResult"))
...
private suspend fun throwAttachVpnGatewayError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
