...
internal class GetConsoleOutputOperationDeserializer: HttpDeserialize<GetConsoleOutputResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetConsoleOutputResponse {
        if (!response.status.isSuccess()) {
            throwGetConsoleOutputError(context, response)
        }
        val builder = GetConsoleOutputResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetConsoleOutputOperationBody(builder, payload)
        }
...
private suspend fun deserializeGetConsoleOutputOperationBody(builder: GetConsoleOutputResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("GetConsoleOutputResult"))
...
private suspend fun throwGetConsoleOutputError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
