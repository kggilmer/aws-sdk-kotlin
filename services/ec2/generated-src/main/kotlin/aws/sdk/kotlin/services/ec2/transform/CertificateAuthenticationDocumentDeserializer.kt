...
internal suspend fun deserializeCertificateAuthenticationDocument(deserializer: Deserializer): CertificateAuthentication {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("CertificateAuthentication"))
        trait(XmlNamespace("http://ec2.amazonaws.com/doc/2016-11-15"))
        field(CLIENTROOTCERTIFICATECHAIN_DESCRIPTOR)
    }

    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                CLIENTROOTCERTIFICATECHAIN_DESCRIPTOR.index -> builder.clientRootCertificateChain = deserializeString()
                null -> break@loop
                else -> skipValue()
            }
        }
