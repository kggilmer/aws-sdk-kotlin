...
internal suspend fun deserializeSpotMaintenanceStrategiesDocument(deserializer: Deserializer): SpotMaintenanceStrategies {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("SpotMaintenanceStrategies"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                CAPACITYREBALANCE_DESCRIPTOR.index -> builder.capacityRebalance = deserializeSpotCapacityRebalanceDocument(deserializer)
                null -> break@loop
                else -> skipValue()
            }
        }
