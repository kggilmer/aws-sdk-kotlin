...
internal suspend fun deserializeDisableFastSnapshotRestoreErrorItemDocument(deserializer: Deserializer): DisableFastSnapshotRestoreErrorItem {
    val builder = DisableFastSnapshotRestoreErrorItem.builder()
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("DisableFastSnapshotRestoreErrorItem"))
            ...
            when (findNextFieldIndex()) {
                FASTSNAPSHOTRESTORESTATEERRORS_DESCRIPTOR.index -> builder.fastSnapshotRestoreStateErrors =
                    deserializer.deserializeList(FASTSNAPSHOTRESTORESTATEERRORS_DESCRIPTOR) {
                        val col0 = mutableListOf<DisableFastSnapshotRestoreStateErrorItem>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializeDisableFastSnapshotRestoreStateErrorItemDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
