...
internal class DescribeAvailabilityZonesOperationDeserializer: HttpDeserialize<DescribeAvailabilityZonesResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DescribeAvailabilityZonesResponse {
        if (!response.status.isSuccess()) {
            throwDescribeAvailabilityZonesError(context, response)
        }
        val builder = DescribeAvailabilityZonesResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDescribeAvailabilityZonesOperationBody(builder, payload)
        }
...
private suspend fun deserializeDescribeAvailabilityZonesOperationBody(builder: DescribeAvailabilityZonesResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("DescribeAvailabilityZonesResult"))
...
private suspend fun throwDescribeAvailabilityZonesError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
