...
import aws.sdk.kotlin.services.ec2.model.FpgaDeviceMemoryInfo
...
internal suspend fun deserializeFpgaDeviceMemoryInfoDocument(deserializer: Deserializer): FpgaDeviceMemoryInfo {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("FpgaDeviceMemoryInfo"))
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                SIZEINMIB_DESCRIPTOR.index -> builder.sizeInMib = deserializeInt()
                null -> break@loop
                else -> skipValue()
            }
        }
