...
internal class EnableEbsEncryptionByDefaultOperationDeserializer: HttpDeserialize<EnableEbsEncryptionByDefaultResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): EnableEbsEncryptionByDefaultResponse {
        if (!response.status.isSuccess()) {
            throwEnableEbsEncryptionByDefaultError(context, response)
        }
        val builder = EnableEbsEncryptionByDefaultResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeEnableEbsEncryptionByDefaultOperationBody(builder, payload)
        }
...
private suspend fun deserializeEnableEbsEncryptionByDefaultOperationBody(builder: EnableEbsEncryptionByDefaultResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("EnableEbsEncryptionByDefaultResult"))
...
private suspend fun throwEnableEbsEncryptionByDefaultError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
