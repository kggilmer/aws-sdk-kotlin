...
internal class DescribeFpgaImagesOperationSerializer: HttpSerialize<DescribeFpgaImagesRequest> {
    override suspend fun serialize(context: ExecutionContext, input: DescribeFpgaImagesRequest): HttpRequestBuilder {
        ...
        val payload = serializeDescribeFpgaImagesOperationBody(context, input)
...
private fun serializeDescribeFpgaImagesOperationBody(context: ExecutionContext, input: DescribeFpgaImagesRequest): ByteArray {
    ...
    val FPGAIMAGEIDS_DESCRIPTOR = SdkFieldDescriptor(SerialKind.List, FormUrlSerialName("FpgaImageId"), FormUrlFlattened)
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(FormUrlSerialName("DescribeFpgaImagesRequest"))
        trait(QueryLiteral("Action", "DescribeFpgaImages"))
        trait(QueryLiteral("Version", "2016-11-15"))
        field(DRYRUN_DESCRIPTOR)
        field(FILTERS_DESCRIPTOR)
        field(FPGAIMAGEIDS_DESCRIPTOR)
    ...
    serializer.serializeStruct(OBJ_DESCRIPTOR) {
        ...
        if (input.fpgaImageIds != null) {
            listField(FPGAIMAGEIDS_DESCRIPTOR) {
                for (el0 in input.fpgaImageIds) {
                    serializeString(el0)
                }
            }
        }
