...
internal class GetObjectRetentionOperationDeserializer: HttpDeserialize<GetObjectRetentionResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetObjectRetentionResponse {
        if (!response.status.isSuccess()) {
            throwGetObjectRetentionError(context, response)
        }
        val builder = GetObjectRetentionResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetObjectRetentionOperationBody(builder, payload)
        }
...
private suspend fun throwGetObjectRetentionError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
