...
internal class DeleteBucketReplicationOperationDeserializer: HttpDeserialize<DeleteBucketReplicationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketReplicationResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketReplicationError(context, response)
        }
        val builder = DeleteBucketReplicationResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketReplicationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
