...
internal class DeleteBucketLifecycleOperationDeserializer: HttpDeserialize<DeleteBucketLifecycleResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketLifecycleResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketLifecycleError(context, response)
        }
        val builder = DeleteBucketLifecycleResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketLifecycleError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
