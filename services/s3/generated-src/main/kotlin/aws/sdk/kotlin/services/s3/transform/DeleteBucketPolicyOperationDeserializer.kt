...
internal class DeleteBucketPolicyOperationDeserializer: HttpDeserialize<DeleteBucketPolicyResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketPolicyResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketPolicyError(context, response)
        }
        val builder = DeleteBucketPolicyResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketPolicyError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
