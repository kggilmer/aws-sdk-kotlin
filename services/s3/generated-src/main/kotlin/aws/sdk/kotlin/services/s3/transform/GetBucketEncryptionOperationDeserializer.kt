...
internal class GetBucketEncryptionOperationDeserializer: HttpDeserialize<GetBucketEncryptionResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketEncryptionResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketEncryptionError(context, response)
        }
        val builder = GetBucketEncryptionResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketEncryptionOperationBody(builder, payload)
        }
...
private suspend fun throwGetBucketEncryptionError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
