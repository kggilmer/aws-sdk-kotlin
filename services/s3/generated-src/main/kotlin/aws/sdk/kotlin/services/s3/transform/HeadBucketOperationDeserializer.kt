...
internal class HeadBucketOperationDeserializer: HttpDeserialize<HeadBucketResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): HeadBucketResponse {
        if (!response.status.isSuccess()) {
            throwHeadBucketError(context, response)
        }
        val builder = HeadBucketResponse.builder()

        return builder.build()
    }
}

private suspend fun throwHeadBucketError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
