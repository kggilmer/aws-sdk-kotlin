...
internal class GetBucketOwnershipControlsOperationDeserializer: HttpDeserialize<GetBucketOwnershipControlsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketOwnershipControlsResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketOwnershipControlsError(context, response)
        }
        val builder = GetBucketOwnershipControlsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketOwnershipControlsOperationBody(builder, payload)
        }
...
private suspend fun throwGetBucketOwnershipControlsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
