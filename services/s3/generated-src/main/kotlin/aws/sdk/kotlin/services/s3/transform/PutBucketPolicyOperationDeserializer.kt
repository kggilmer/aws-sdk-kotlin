...
internal class PutBucketPolicyOperationDeserializer: HttpDeserialize<PutBucketPolicyResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketPolicyResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketPolicyError(context, response)
        }
        val builder = PutBucketPolicyResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketPolicyError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
