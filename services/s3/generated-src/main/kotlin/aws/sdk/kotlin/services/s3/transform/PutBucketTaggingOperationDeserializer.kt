...
internal class PutBucketTaggingOperationDeserializer: HttpDeserialize<PutBucketTaggingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketTaggingResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketTaggingError(context, response)
        }
        val builder = PutBucketTaggingResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketTaggingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
