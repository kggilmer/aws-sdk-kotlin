...
internal class DeleteBucketWebsiteOperationDeserializer: HttpDeserialize<DeleteBucketWebsiteResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketWebsiteResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketWebsiteError(context, response)
        }
        val builder = DeleteBucketWebsiteResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketWebsiteError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
