...
internal class GetPublicAccessBlockOperationDeserializer: HttpDeserialize<GetPublicAccessBlockResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetPublicAccessBlockResponse {
        if (!response.status.isSuccess()) {
            throwGetPublicAccessBlockError(context, response)
        }
        val builder = GetPublicAccessBlockResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetPublicAccessBlockOperationBody(builder, payload)
        }
...
private suspend fun throwGetPublicAccessBlockError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
