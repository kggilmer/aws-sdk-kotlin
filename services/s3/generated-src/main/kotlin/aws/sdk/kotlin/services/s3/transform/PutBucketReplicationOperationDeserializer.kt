...
internal class PutBucketReplicationOperationDeserializer: HttpDeserialize<PutBucketReplicationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketReplicationResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketReplicationError(context, response)
        }
        val builder = PutBucketReplicationResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketReplicationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
