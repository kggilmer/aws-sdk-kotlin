...
internal class CreateMultipartUploadOperationDeserializer: HttpDeserialize<CreateMultipartUploadResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateMultipartUploadResponse {
        if (!response.status.isSuccess()) {
            throwCreateMultipartUploadError(context, response)
        }
        val builder = CreateMultipartUploadResponse.builder()
        ...
        if (payload != null) {
            deserializeCreateMultipartUploadOperationBody(builder, payload)
        }
...
private suspend fun deserializeCreateMultipartUploadOperationBody(builder: CreateMultipartUploadResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("InitiateMultipartUploadResult"))
...
private suspend fun throwCreateMultipartUploadError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
