...
internal class GetObjectTaggingOperationDeserializer: HttpDeserialize<GetObjectTaggingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetObjectTaggingResponse {
        if (!response.status.isSuccess()) {
            throwGetObjectTaggingError(context, response)
        }
        val builder = GetObjectTaggingResponse.builder()

        builder.versionId = response.headers["x-amz-version-id"]

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetObjectTaggingOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializeGetObjectTaggingOperationBody(builder: GetObjectTaggingResponse.DslBuilder, payload: ByteArray) {
...
private suspend fun throwGetObjectTaggingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
