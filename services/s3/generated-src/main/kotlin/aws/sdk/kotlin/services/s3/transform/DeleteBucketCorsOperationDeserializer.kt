...
internal class DeleteBucketCorsOperationDeserializer: HttpDeserialize<DeleteBucketCorsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketCorsResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketCorsError(context, response)
        }
        val builder = DeleteBucketCorsResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketCorsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
