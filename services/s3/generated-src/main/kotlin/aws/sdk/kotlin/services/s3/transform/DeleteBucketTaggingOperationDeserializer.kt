...
internal class DeleteBucketTaggingOperationDeserializer: HttpDeserialize<DeleteBucketTaggingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketTaggingResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketTaggingError(context, response)
        }
        val builder = DeleteBucketTaggingResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketTaggingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
