...
internal class PutBucketAclOperationDeserializer: HttpDeserialize<PutBucketAclResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketAclResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketAclError(context, response)
        }
        val builder = PutBucketAclResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketAclError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
