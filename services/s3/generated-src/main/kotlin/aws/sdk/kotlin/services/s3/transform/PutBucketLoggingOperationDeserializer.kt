...
internal class PutBucketLoggingOperationDeserializer: HttpDeserialize<PutBucketLoggingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketLoggingResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketLoggingError(context, response)
        }
        val builder = PutBucketLoggingResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketLoggingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
