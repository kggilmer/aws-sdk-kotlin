...
internal class GetObjectAclOperationDeserializer: HttpDeserialize<GetObjectAclResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetObjectAclResponse {
        if (!response.status.isSuccess()) {
            throwGetObjectAclError(context, response)
        }
        val builder = GetObjectAclResponse.builder()

        builder.requestCharged = response.headers["x-amz-request-charged"]?.let { RequestCharged.fromValue(it) }

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetObjectAclOperationBody(builder, payload)
        }
...
private suspend fun throwGetObjectAclError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
    ...
    val modeledExceptionDeserializer = when(errorDetails.code) {
        "NoSuchKey" -> NoSuchKeyDeserializer()
