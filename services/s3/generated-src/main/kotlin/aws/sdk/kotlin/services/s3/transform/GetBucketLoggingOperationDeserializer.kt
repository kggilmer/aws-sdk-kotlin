...
internal class GetBucketLoggingOperationDeserializer: HttpDeserialize<GetBucketLoggingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketLoggingResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketLoggingError(context, response)
        }
        val builder = GetBucketLoggingResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketLoggingOperationBody(builder, payload)
        }
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                LOGGINGENABLED_DESCRIPTOR.index -> builder.loggingEnabled = deserializeLoggingEnabledDocument(deserializer)
                null -> break@loop
                else -> skipValue()
            }
        }
...
private suspend fun throwGetBucketLoggingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
