...
internal class GetBucketPolicyOperationDeserializer: HttpDeserialize<GetBucketPolicyResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketPolicyResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketPolicyError(context, response)
        }
        val builder = GetBucketPolicyResponse.builder()

        val contents = response.body.readAll()?.decodeToString()
        builder.policy = contents

        return builder.build()
    }
}

private suspend fun throwGetBucketPolicyError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
