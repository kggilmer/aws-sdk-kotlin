...
internal class GetObjectLegalHoldOperationDeserializer: HttpDeserialize<GetObjectLegalHoldResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetObjectLegalHoldResponse {
        if (!response.status.isSuccess()) {
            throwGetObjectLegalHoldError(context, response)
        }
        val builder = GetObjectLegalHoldResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetObjectLegalHoldOperationBody(builder, payload)
        }
...
private suspend fun throwGetObjectLegalHoldError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
