...
internal class PutBucketCorsOperationDeserializer: HttpDeserialize<PutBucketCorsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutBucketCorsResponse {
        if (!response.status.isSuccess()) {
            throwPutBucketCorsError(context, response)
        }
        val builder = PutBucketCorsResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutBucketCorsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
