...
internal class GetBucketMetricsConfigurationOperationDeserializer: HttpDeserialize<GetBucketMetricsConfigurationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketMetricsConfigurationResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketMetricsConfigurationError(context, response)
        }
        val builder = GetBucketMetricsConfigurationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketMetricsConfigurationOperationBody(builder, payload)
        }
...
private suspend fun throwGetBucketMetricsConfigurationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
