...
internal class ListBucketInventoryConfigurationsOperationDeserializer: HttpDeserialize<ListBucketInventoryConfigurationsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): ListBucketInventoryConfigurationsResponse {
        if (!response.status.isSuccess()) {
            throwListBucketInventoryConfigurationsError(context, response)
        }
...
private suspend fun deserializeListBucketInventoryConfigurationsOperationBody(builder: ListBucketInventoryConfigurationsResponse.DslBuilder, payload: ByteArray) {
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("ListInventoryConfigurationsResult"))
        trait(XmlNamespace("http://s3.amazonaws.com/doc/2006-03-01/"))
        field(CONTINUATIONTOKEN_DESCRIPTOR)
        field(INVENTORYCONFIGURATIONLIST_DESCRIPTOR)
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                ...
                INVENTORYCONFIGURATIONLIST_DESCRIPTOR.index -> builder.inventoryConfigurationList =
                    deserializer.deserializeList(INVENTORYCONFIGURATIONLIST_DESCRIPTOR) {
                        val col0 = mutableListOf<InventoryConfiguration>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializeInventoryConfigurationDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
...
private suspend fun throwListBucketInventoryConfigurationsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
