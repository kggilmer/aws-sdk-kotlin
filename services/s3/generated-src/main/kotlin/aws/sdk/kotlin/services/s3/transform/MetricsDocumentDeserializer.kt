    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                EVENTTHRESHOLD_DESCRIPTOR.index -> builder.eventThreshold = deserializeReplicationTimeValueDocument(deserializer)
                STATUS_DESCRIPTOR.index -> builder.status = deserializeString().let { MetricsStatus.fromValue(it) }
                null -> break@loop
                else -> skipValue()
            }
        }
