    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while(true) {
            when(findNextFieldIndex()) {
                AND_DESCRIPTOR.index -> value = AnalyticsFilter.And(deserializeAnalyticsAndOperatorDocument(deserializer))
                PREFIX_DESCRIPTOR.index -> value = AnalyticsFilter.Prefix(deserializeString())
                TAG_DESCRIPTOR.index -> value = AnalyticsFilter.Tag(deserializeTagDocument(deserializer))
                null -> break@loop
                else -> value = AnalyticsFilter.SdkUnknown.also { skipValue() }
            }
        }
    }
    return value ?: throw DeserializationException("Deserialized value unexpectedly null: AnalyticsFilter")
}
