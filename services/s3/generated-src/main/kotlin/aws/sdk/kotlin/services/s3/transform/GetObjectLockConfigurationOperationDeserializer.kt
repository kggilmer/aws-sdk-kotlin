...
internal class GetObjectLockConfigurationOperationDeserializer: HttpDeserialize<GetObjectLockConfigurationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetObjectLockConfigurationResponse {
        if (!response.status.isSuccess()) {
            throwGetObjectLockConfigurationError(context, response)
        }
        val builder = GetObjectLockConfigurationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetObjectLockConfigurationOperationBody(builder, payload)
        }
...
private suspend fun throwGetObjectLockConfigurationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
