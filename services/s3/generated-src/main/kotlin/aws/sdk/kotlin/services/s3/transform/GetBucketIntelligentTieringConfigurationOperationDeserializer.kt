...
internal class GetBucketIntelligentTieringConfigurationOperationDeserializer: HttpDeserialize<GetBucketIntelligentTieringConfigurationResponse> {
    ...
    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketIntelligentTieringConfigurationResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketIntelligentTieringConfigurationError(context, response)
        }
        val builder = GetBucketIntelligentTieringConfigurationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketIntelligentTieringConfigurationOperationBody(builder, payload)
        }
...
private suspend fun throwGetBucketIntelligentTieringConfigurationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
