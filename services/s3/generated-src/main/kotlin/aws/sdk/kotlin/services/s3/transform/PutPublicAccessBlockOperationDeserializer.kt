...
internal class PutPublicAccessBlockOperationDeserializer: HttpDeserialize<PutPublicAccessBlockResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutPublicAccessBlockResponse {
        if (!response.status.isSuccess()) {
            throwPutPublicAccessBlockError(context, response)
        }
        val builder = PutPublicAccessBlockResponse.builder()

        return builder.build()
    }
}

private suspend fun throwPutPublicAccessBlockError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
