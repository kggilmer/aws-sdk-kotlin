...
internal class RestoreObjectOperationDeserializer: HttpDeserialize<RestoreObjectResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): RestoreObjectResponse {
        if (!response.status.isSuccess()) {
            throwRestoreObjectError(context, response)
        }
        val builder = RestoreObjectResponse.builder()

        builder.requestCharged = response.headers["x-amz-request-charged"]?.let { RequestCharged.fromValue(it) }
        builder.restoreOutputPath = response.headers["x-amz-restore-output-path"]

        return builder.build()
    }
}

private suspend fun throwRestoreObjectError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    ...
    val modeledExceptionDeserializer = when(errorDetails.code) {
        "ObjectAlreadyInActiveTierError" -> ObjectAlreadyInActiveTierErrorDeserializer()
