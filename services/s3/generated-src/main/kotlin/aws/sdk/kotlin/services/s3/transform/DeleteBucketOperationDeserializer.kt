...
internal class DeleteBucketOperationDeserializer: HttpDeserialize<DeleteBucketResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteBucketResponse {
        if (!response.status.isSuccess()) {
            throwDeleteBucketError(context, response)
        }
        val builder = DeleteBucketResponse.builder()

        return builder.build()
    }
}

private suspend fun throwDeleteBucketError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
