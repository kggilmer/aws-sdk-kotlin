...
internal class CreateBucketOperationDeserializer: HttpDeserialize<CreateBucketResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateBucketResponse {
        if (!response.status.isSuccess()) {
            throwCreateBucketError(context, response)
        }
        val builder = CreateBucketResponse.builder()
...
private suspend fun throwCreateBucketError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    ...
    val modeledExceptionDeserializer = when(errorDetails.code) {
        "BucketAlreadyExists" -> BucketAlreadyExistsDeserializer()
        "BucketAlreadyOwnedByYou" -> BucketAlreadyOwnedByYouDeserializer()
