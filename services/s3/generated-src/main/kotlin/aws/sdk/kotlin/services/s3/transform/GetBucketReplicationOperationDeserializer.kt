...
internal class GetBucketReplicationOperationDeserializer: HttpDeserialize<GetBucketReplicationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketReplicationResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketReplicationError(context, response)
        }
        val builder = GetBucketReplicationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketReplicationOperationBody(builder, payload)
        }
...
private suspend fun throwGetBucketReplicationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
