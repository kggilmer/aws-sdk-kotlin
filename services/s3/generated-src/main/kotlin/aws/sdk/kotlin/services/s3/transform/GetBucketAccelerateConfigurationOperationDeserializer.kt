...
import aws.sdk.kotlin.services.s3.model.BucketAccelerateStatus
...
internal class GetBucketAccelerateConfigurationOperationDeserializer: HttpDeserialize<GetBucketAccelerateConfigurationResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetBucketAccelerateConfigurationResponse {
        if (!response.status.isSuccess()) {
            throwGetBucketAccelerateConfigurationError(context, response)
        }
        val builder = GetBucketAccelerateConfigurationResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetBucketAccelerateConfigurationOperationBody(builder, payload)
        }
    ...
    deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
        loop@while (true) {
            when (findNextFieldIndex()) {
                STATUS_DESCRIPTOR.index -> builder.status = deserializeString().let { BucketAccelerateStatus.fromValue(it) }
                null -> break@loop
                else -> skipValue()
            }
        }
...
private suspend fun throwGetBucketAccelerateConfigurationError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
