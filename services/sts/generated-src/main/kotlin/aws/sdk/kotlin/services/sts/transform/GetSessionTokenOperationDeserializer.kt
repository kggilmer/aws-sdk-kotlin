...
internal class GetSessionTokenOperationDeserializer: HttpDeserialize<GetSessionTokenResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): GetSessionTokenResponse {
        if (!response.status.isSuccess()) {
            throwGetSessionTokenError(context, response)
        }
        val builder = GetSessionTokenResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeGetSessionTokenOperationBody(builder, payload)
        }
...
private suspend fun deserializeGetSessionTokenOperationBody(builder: GetSessionTokenResponse.DslBuilder, payload: ByteArray) {
    ...
    val wrapperDescriptor = SdkObjectDescriptor.build {
        trait(XmlSerialName("GetSessionTokenResponse"))
        field(resultDescriptor)
    }

    val wrapper = deserializer.deserializeStruct(wrapperDescriptor)
    if (wrapper.findNextFieldIndex() != resultDescriptor.index) {
        throw DeserializationException("failed to unwrap GetSessionToken response")
    }
    ...
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        ...
        trait(XmlSerialName("GetSessionTokenResult"))
...
private suspend fun throwGetSessionTokenError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
