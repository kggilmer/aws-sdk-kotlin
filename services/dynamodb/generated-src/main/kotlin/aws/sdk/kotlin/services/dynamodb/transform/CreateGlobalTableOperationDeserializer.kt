...
internal class CreateGlobalTableOperationDeserializer: HttpDeserialize<CreateGlobalTableResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): CreateGlobalTableResponse {
        if (!response.status.isSuccess()) {
            throwCreateGlobalTableError(context, response)
        }
        val builder = CreateGlobalTableResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeCreateGlobalTableOperationBody(builder, payload)
        }
...
private suspend fun throwCreateGlobalTableError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
    ...
    val modeledExceptionDeserializer = when(errorDetails.code) {
        "GlobalTableAlreadyExistsException" -> GlobalTableAlreadyExistsExceptionDeserializer()
