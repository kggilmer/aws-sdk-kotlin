...
internal class UpdateTableReplicaAutoScalingOperationDeserializer: HttpDeserialize<UpdateTableReplicaAutoScalingResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): UpdateTableReplicaAutoScalingResponse {
        if (!response.status.isSuccess()) {
            throwUpdateTableReplicaAutoScalingError(context, response)
        }
        val builder = UpdateTableReplicaAutoScalingResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeUpdateTableReplicaAutoScalingOperationBody(builder, payload)
        }
        return builder.build()
    }
}

private suspend fun deserializeUpdateTableReplicaAutoScalingOperationBody(builder: UpdateTableReplicaAutoScalingResponse.DslBuilder, payload: ByteArray) {
...
private suspend fun throwUpdateTableReplicaAutoScalingError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
