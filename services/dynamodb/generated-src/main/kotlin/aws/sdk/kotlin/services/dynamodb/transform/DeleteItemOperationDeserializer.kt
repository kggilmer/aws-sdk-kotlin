...
internal class DeleteItemOperationDeserializer: HttpDeserialize<DeleteItemResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): DeleteItemResponse {
        if (!response.status.isSuccess()) {
            throwDeleteItemError(context, response)
        }
        val builder = DeleteItemResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeDeleteItemOperationBody(builder, payload)
        }
...
private suspend fun throwDeleteItemError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
