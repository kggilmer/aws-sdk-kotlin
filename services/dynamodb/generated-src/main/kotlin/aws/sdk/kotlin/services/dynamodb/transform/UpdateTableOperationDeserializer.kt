...
internal class UpdateTableOperationDeserializer: HttpDeserialize<UpdateTableResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): UpdateTableResponse {
        if (!response.status.isSuccess()) {
            throwUpdateTableError(context, response)
        }
        val builder = UpdateTableResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeUpdateTableOperationBody(builder, payload)
        }
...
private suspend fun throwUpdateTableError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
