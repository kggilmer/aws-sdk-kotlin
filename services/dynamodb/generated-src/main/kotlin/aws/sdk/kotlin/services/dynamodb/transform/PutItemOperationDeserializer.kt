...
internal class PutItemOperationDeserializer: HttpDeserialize<PutItemResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): PutItemResponse {
        if (!response.status.isSuccess()) {
            throwPutItemError(context, response)
        }
        val builder = PutItemResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializePutItemOperationBody(builder, payload)
        }
...
private suspend fun throwPutItemError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
