...
internal class UpdateGlobalTableSettingsOperationDeserializer: HttpDeserialize<UpdateGlobalTableSettingsResponse> {

    override suspend fun deserialize(context: ExecutionContext, response: HttpResponse): UpdateGlobalTableSettingsResponse {
        if (!response.status.isSuccess()) {
            throwUpdateGlobalTableSettingsError(context, response)
        }
        val builder = UpdateGlobalTableSettingsResponse.builder()

        val payload = response.body.readAll()
        if (payload != null) {
            deserializeUpdateGlobalTableSettingsOperationBody(builder, payload)
        }
...
private suspend fun throwUpdateGlobalTableSettingsError(context: ExecutionContext, response: HttpResponse): kotlin.Nothing {
    val payload = response.body.readAll()
    ...
    val modeledExceptionDeserializer = when(errorDetails.code) {
        ...
        "IndexNotFoundException" -> IndexNotFoundExceptionDeserializer()
